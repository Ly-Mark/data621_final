...
```{r}
dementia <- read.csv("D:/UofC2022/Data621/Project/Data/oasis_cross-sectional.csv")

dementia <- dementia %>% dplyr::select(-c(Hand,Delay))


vis_miss(dementia)
gg_miss_upset(dementia, 
              nsets = 10,
              nintersects = 50)

dementia_sub <- dementia %>% drop_na() # 216


dementia_sum <- dementia_sub[3:10]
dementia_summary<-as.data.frame(apply(dementia_sum,2,summary))
dementia_summary
```

```{r,fig.width=5,fig.height=5}
def_par <- fp_par(text.align = "center", padding = 5)
def_text <- fp_text(font.size = 11, font.family = "Arial")


ft <- flextable(data= dementia_summary)
ft <- add_header(ft,
                 Age= "Continuous",
                 Educ= "Categorical",
                 SES= "Categorical",
                 ...
                 eTIV= "Continuous",
                 nWBV= "Continuous",
                 ASF= "Continuous",top = FALSE)

ft <- add_header_row(
    x=ft,
    values = c('Demongraphic','Clinical','Derrived Anatomic Values'),
    colwidths = c(3,2,3))
...
ft <- style(x = ft, pr_p = def_par, pr_t = def_text, part = "all") 
ft <- autofit(ft)
...
border <- fp_border()
vline(ft, j = c('SES', 'CDR'), border = border, part = "all")
```

```{r}
dementia_full <- ggplot(dementia, aes(x=Age))
dementia_full <- dementia_full + geom_histogram(binwidth=5, colour="black", 
                          aes(y=..density.., fill=..count..))
dementia_full <- dementia_full + scale_fill_gradient("Count")
dementia_full <- dementia_full + stat_function(fun=dnorm,color="red",
                                                 args=list(mean=mean(dementia$Age),
                                                           sd=sd(dementia$Age)))
dementia_filter <- ggplot(dementia_sub, aes(x=Age))
dementia_filter <- dementia_filter + geom_histogram(binwidth=5, colour="black", 
                          aes(y=..density.., fill=..count..))
dementia_filter <- dementia_filter + scale_fill_gradient("Count")
dementia_filter <- dementia_filter + stat_function(fun=dnorm,color="red",
                                                 args=list(mean=mean(dementia_sub$Age),
                                                           sd=sd(dementia_sub$Age)))

par(mfrow=c(1,2))

dementia_full<-dementia_full + labs(x = "Age",
                  y = "Density",
                  title = "Distribution of Patient's Age (Full)")   
dementia_filter<-dementia_filter + labs(x = "Age",
                  y = "Density",
                  title = "Distribution of Patient's Age (Filter)")   

grid.arrange(dementia_full, dementia_filter, ncol=2)
```
...
![Dag ](D:/UofC2022/Data621/Project/Images/dagitty-model2.png)
